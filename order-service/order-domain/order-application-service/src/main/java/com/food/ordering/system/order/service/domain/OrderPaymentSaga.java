  ...
  public void rollback(PaymentResponse paymentResponse) {
    ...
    if (paymentResponse.getPaymentStatus() == PaymentStatus.CANCELLED) {
      approvalOutboxHelper.save(
          getUpdatedApprovalOutboxMessage(paymentResponse.getSagaId(), order.getOrderStatus(),
              sagaStatus));
    }
  ...
  private OrderApprovalOutboxMessage getUpdatedApprovalOutboxMessage(String sagaId,
    ...
    if (orderApprovalOutboxMessageResponse.isEmpty()) {
      throw new OrderDomainException(
          "Approval outbox message could not be found in " + SagaStatus.COMPENSATING.name());
    }
