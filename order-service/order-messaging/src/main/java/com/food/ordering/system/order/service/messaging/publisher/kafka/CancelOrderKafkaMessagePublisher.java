...
import com.food.ordering.system.order.servide.domain.ports.output.message.publisher.payment.OrderCancelledPaymentRequestMessagePublisher;
...
public class CancelOrderKafkaMessagePublisher implements
  ...
  @Override
  public void publish(OrderCancelledEvent domain) {
    String orderId = domain.getOrder().getId().getValue().toString();
    log.info("Received OrderCancelledEvent for order id: {}", orderId);
    try {
      PaymentRequestAvroModel paymentRequestAvroModel =
          orderMessagingDataMapper.orderCancelledEventToPaymentRequestAvroModel(domain);
